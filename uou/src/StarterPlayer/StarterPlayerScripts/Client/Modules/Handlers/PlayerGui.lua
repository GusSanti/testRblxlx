...
local UseSkillRemote = GameRemotes:WaitForChild("UseSkill")
...
local isTargetingSkill = false
local targetingCircle = nil
local targetRenderConn = nil
local targetClickConn = nil
local targetCancelConn = nil
...
local function cancelSkillTargeting()
	isTargetingSkill = false
	if targetingCircle then
		targetingCircle:Destroy()
		targetingCircle = nil
	end
	if targetRenderConn then
		targetRenderConn:Disconnect()
		targetRenderConn = nil
	end
	if targetClickConn then
		targetClickConn:Disconnect()
		targetClickConn = nil
	end
	if targetCancelConn then
		targetCancelConn:Disconnect()
		targetCancelConn = nil
	end
end

local function startLocalCooldown(slotName, skillButton, skillData)
	LocalCooldowns[slotName] = true
	local cooldownImage = skillButton:FindFirstChild("CooldownImage")
	if cooldownImage then
		local aspectConstraint = cooldownImage:FindFirstChildOfClass("UIAspectRatioConstraint")
		if aspectConstraint then
			aspectConstraint:Destroy()
		end

		cooldownImage.AnchorPoint = Vector2.new(0.5, 1)
		cooldownImage.Position = UDim2.new(0.5, 0, 1, 0)

		cooldownImage.Visible = true
		cooldownImage.Size = UDim2.new(1, 0, 1, 0)

		local tweenInfo = TweenInfo.new(skillData.Cooldown, Enum.EasingStyle.Linear)
		local tween = TweenService:Create(cooldownImage, tweenInfo, {Size = UDim2.new(1, 0, 0, 0)})
		tween:Play()
		tween.Completed:Connect(function()
			cooldownImage.Visible = false
			LocalCooldowns[slotName] = nil
		end)
	end
end

local function checkHasTowerLocally(towerName)
	local PlacedTowersFolder = workspace:FindFirstChild("Towers")
	if not PlacedTowersFolder then return false end

	for _, tower in ipairs(PlacedTowersFolder:GetChildren()) do
		if tower.Name:find(towerName) and tower:GetAttribute("Owner") == Player.UserId then
			return true
		end
	end
...
local function enterSkillTargeting(slotName, towerName, skillButton, skillData)
	isTargetingSkill = true

	local WhiteCircleTemplate = ReplicatedStorage.Storage.Circles:FindFirstChild("WhiteCircle")
	if WhiteCircleTemplate then
		targetingCircle = WhiteCircleTemplate:Clone()
		targetingCircle.Size = Vector3.new(0.375, skillData.Range * 2, skillData.Range * 2)
		targetingCircle.Parent = workspace
		targetingCircle.Anchored = true
		targetingCircle.CanCollide = false
		targetingCircle.CanQuery = false
	end

	local mouse = Player:GetMouse()

	targetRenderConn = RunService.RenderStepped:Connect(function()
		if targetingCircle then
			local targetPos = mouse.Hit.Position
			targetingCircle.CFrame = CFrame.new(targetPos) * CFrame.Angles(0, 0, math.pi / 2)

			if mouse.Target then
				targetingCircle.SurfaceGui.Frame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
				targetingCircle.SurfaceGui.Frame.UIStroke.Color = Color3.fromRGB(255, 255, 255)
			else
				targetingCircle.SurfaceGui.Frame.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
				targetingCircle.SurfaceGui.Frame.UIStroke.Color = Color3.fromRGB(255, 0, 0)
			end
		end
	end)

	targetClickConn = mouse.Button1Down:Connect(function()
		if mouse.Target then
			local targetPos = mouse.Hit.Position
			UseSkillRemote:FireServer(slotName, towerName, targetPos)
			startLocalCooldown(slotName, skillButton, skillData)
			cancelSkillTargeting()
		end
	end)

	targetCancelConn = mouse.Button2Down:Connect(function()
		cancelSkillTargeting()
	end)
end

local function triggerSkill(slotName, towerName, skillButton, skillData)
	if LocalCooldowns[slotName] then return end

	if not checkHasTowerLocally(towerName) then
		sendNotification("You need to place this tower first!", "Error")
		return
	end

	if skillData.RequiresTarget then
		if isTargetingSkill then
			cancelSkillTargeting()
		end
		enterSkillTargeting(slotName, towerName, skillButton, skillData)
	else
		UseSkillRemote:FireServer(slotName, towerName)
		startLocalCooldown(slotName, skillButton, skillData)
	end
end
...
local function setupHotbar()
	...
	for _, Slot in ipairs(InventoryFolder:GetChildren()) do
		...
		if Slot.Value == "" then 
			...
			if SkillButton then SkillButton.Visible = false end
			continue 
		end
		...
		if SkillButton then
			local SkillData = TowerData[Slot.Value].Skill
			if SkillData then
				SkillButton.Visible = true

				local ImageSkill = SkillButton:FindFirstChild("ImageSkill")
				if ImageSkill then
					ImageSkill.Image = "rbxassetid://"..SkillData.ImageId
				end

				local CooldownImage = SkillButton:FindFirstChild("CooldownImage")
				if CooldownImage then
					CooldownImage.Visible = false
					CooldownImage.Size = UDim2.new(1, 0, 0, 0)
				end

				SkillButton.MouseButton1Click:Connect(function()
					triggerSkill(SlotNumber, Slot.Value, SkillButton, SkillData)
				end)
			else
...
local function handleClientTowers()
	...
	mouse.Button1Down:Connect(function()
		if isTargetingSkill then return end
		local target = mouse.Target

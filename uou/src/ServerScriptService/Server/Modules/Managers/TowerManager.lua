...
local SkillBehaviors = {
	["Explosion"] = function(Player, TowerName, SkillData, TargetPosition)
		local hasTower = false
		local towersOfPlayer = playerTowers[Player] or {}

		for _, tower in ipairs(towersOfPlayer) do
			if tower.Name:find(TowerName) and tower.PrimaryPart then
				hasTower = true
				local pos = tower.PrimaryPart.Position

				local ExplosionPart = ExplosionTemplate:Clone()
				ExplosionPart.Position = pos
				ExplosionPart.Parent = workspace

				task.spawn(function()
					task.wait(0.05)
					for _, Particle in ipairs(ExplosionPart:GetDescendants()) do
						if Particle:IsA("ParticleEmitter") then Particle:Emit(10) end
					end
					task.wait(3)
					...
					if enemy:IsA("Model") and enemy.PrimaryPart then
						...
						if distance <= SkillData.Range then
							...
							if humanoid then
								...
								Remotes.Game.VisualDamage:FireAllClients(enemy, SkillData.Damage)

								if oldHealth > 0 and humanoid.Health <= 0 then
									processSkillKill(Player, enemy, enemy.PrimaryPart.Position)
								end
							end
						end
					end
		...
		if not hasTower then
			Remotes.Game.SendNotification:FireClient(Player, "You need to place this tower first!", "Error")
			return false 
		end
	...
	["TargetedExplosion"] = function(Player, TowerName, SkillData, TargetPosition)
		...
		task.spawn(function()
			task.wait(0.05)
			for _, Particle in ipairs(ExplosionPart:GetDescendants()) do
				if Particle:IsA("ParticleEmitter") then Particle:Emit(20) end
			end
			task.wait(3)
			if ExplosionPart then ExplosionPart:Destroy() end
		end)
		...
		if craterFolder then
			...
			for i = 0, PartCount - 1 do
				...
				local partPosition = Vector3.new(x, TargetPosition.Y, z)
...
local function executeSkill(Player, SlotNumber, TowerName, TargetPosition)
	local TData = TowerData[TowerName]
	if not TData or not TData.Skill then return end

	local SkillData = TData.Skill
	...
	local LastUsed = playerSkillCooldowns[Player][SlotNumber] or 0
	...
	if SkillBehaviors[SkillName] then
		local success = SkillBehaviors[SkillName](Player, TowerName, SkillData, TargetPosition)
		if success then
			playerSkillCooldowns[Player][SlotNumber] = CurrentTime
		end
	end
end
...
Remotes:FindFirstChild("Game"):WaitForChild("UseSkill").OnServerEvent:Connect(function(Player, SlotNumber, TowerName, TargetPosition)
	executeSkill(Player, SlotNumber, TowerName, TargetPosition)
end)

...
local Effects = {
	...
	ShiroUltimate = function(Params)
		...
		local function ImpactFrame(arg1, arg2, arg3)
			for _, ImpactFrame in pairs(arg1:GetChildren()) do
				if ImpactFrame.Name == arg2 then
		...
		local function CloneToParents(obj, ...)
			local clones = {}
		...
		local CharCamera2 = Character:FindFirstChild("camera2", true)
		local EnemyCamera2 = Enemy:FindFirstChild("camera2", true)

		-- Efeitos iniciais (imediatos)
		local ScreenLinesClones = CloneToParents(Fx.screen.ScreenEffect["Energy Spec"], CharCamera2, EnemyCamera2)
		local ScreenLinesClones2 = CloneToParents(Fx.screen.ScreenEffect["Energy Spec"], CharCamera2, EnemyCamera2)
		local WindBeamClones = CloneToParents(Fx.screen.ScreenEffectFall.WindBeam, CharCamera2, EnemyCamera2)
		local LinesEnableClones = CloneToParents(Fx.screen.ScreenEffectFall.LinesEnable, CharCamera2, EnemyCamera2)
		local LinesEnable2Clones = CloneToParents(Fx.screen.ScreenEffectFall.LinesEnable2, CharCamera2, EnemyCamera2)

		if CharCamera2 then Utilities.Particle_Setup({Holder = CharCamera2, Type = "Emit"}) end
		if EnemyCamera2 then Utilities.Particle_Setup({Holder = EnemyCamera2, Type = "Emit"}) end

		for _, clone in ipairs(ScreenLinesClones) do Debris:AddItem(clone, 2.4) end
		for _, clone in ipairs(ScreenLinesClones2) do Debris:AddItem(clone, 2.4) end
		for _, clone in ipairs(WindBeamClones) do Debris:AddItem(clone, 2.4) end
		for _, clone in ipairs(LinesEnableClones) do Debris:AddItem(clone, 2.4) end
		for _, clone in ipairs(LinesEnable2Clones) do Debris:AddItem(clone, 2.4) end

		delayCompensated(1.34, function()
			Emit(Torso, Fx.ground.groot, 3, {KeepRotation = true})
			AfterImage.CreateClone(Character, 1, Color3.fromRGB(158, 0, 0), 0.25, 1, 0.6)
		end)

		delayCompensated(1.39, function()
			Emit(Torso, Fx.ground.jump, 3, {KeepRotation = true})
			for _, cam in ipairs({CharCamera2, EnemyCamera2}) do
				if cam then
					local EfeitoVermelhoTorso = Fx.EfeitoVermelhonoChar.Attachment:Clone()
					EfeitoVermelhoTorso.Parent = cam
					Utilities.Particle_Setup({Holder = EfeitoVermelhoTorso, Type = "Emit"})
					Debris:AddItem(EfeitoVermelhoTorso, 1)
				end
			end
		end)

		delayCompensated(2.23, function()
			for _, cam in ipairs({CharCamera2, EnemyCamera2}) do
				if cam then
					local Black = Fx.black.Attachment:Clone()
					Black.Parent = cam
					Debris:AddItem(Black, 0.5)
				end
			end

			ImpactFrame(ImpactFrames, '1', 0.090)
			task.wait(0.1)
			ImpactFrame(ImpactFrames, '2', 0.090)
			task.wait(0.1)
			ImpactFrame(ImpactFrames, '3', 0.090)
		end)

		delayCompensated(3.35, function()
			Emit(Torso, Fx.second.jump, 3, {KeepRotation = true})
		end)

		delayCompensated(4.07, function()
			Emit(enemyTorso, Fx.second.hitup.Sheathe, 3, {KeepRotation = true})
			VFX.Highlight(Enemy, Color3.fromRGB(255, 0, 0), .6)
			AfterImage.CreateClone(Enemy, 1, Color3.fromRGB(255, 255, 255), 0.25, 1, 0.6)
		end)

		delayCompensated(4.15, function()
			Emit(enemyTorso, Fx.second.hitup.impact, 3, {KeepRotation = true})
		end)

		delayCompensated(5.49, function()
			Emit(Torso, Fx.jump.groot, 3, {KeepRotation = true})
			AfterImage.CreateClone(Character, 1, Color3.fromRGB(255, 0, 0), 0.25, 1, 0.6)
		end)

		delayCompensated(6.45, function()
			for _, cam in ipairs({CharCamera2, EnemyCamera2}) do
				if cam then
					local EfeitoVermelhoTorso = Fx.EfeitoVermelhonoChar.Attachment:Clone()
					EfeitoVermelhoTorso.Parent = cam
		...
		delayCompensated(7.35, function()
			Emit(Enemy.Head, Fx.second.sss.impact, 3, {KeepRotation = true})
			VFX.Highlight(Enemy, Color3.fromRGB(255, 0, 0), .6)
		end)

		delayCompensated(8.59, function()
			Emit(Enemy.Head, Fx.second.sss.impact.bleeodd, 3, {KeepRotation = true})
			VFX.Highlight(Enemy, Color3.fromRGB(255, 0, 0), .6)
		end)
		...
		delayCompensated(9.25, function()
			for _, cam in ipairs({CharCamera2, EnemyCamera2}) do
				if cam then
					local Fxscreen = Fx.fall.Start.Attachment:Clone()
					Fxscreen.Parent = cam
					Debris:AddItem(Fxscreen, 0.9)
				end
			end

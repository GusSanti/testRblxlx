...
local Effects = {
	...
	GrimmUltimate = function(Params)
		...
		Emit(Camera2, Fx.winds.Model, 1, {KeepRotation = true})
		Emit(EnemyCamera2, Fx.winds.Model, 1, {KeepRotation = true})
		
		delayCompensated(2.09,function()
			Emit(enemyTorso, Fx.fall.fall, 3)
		end)
		
		delayCompensated(3.7,function()
			for _, cam in ipairs({Camera2, EnemyCamera2}) do
				if cam then
					local Black = Fx.screen.black.Attachment:Clone()
					Black.Parent = cam
		...
		delayCompensated(4.22,function()
			Emit(enemyTorso, Fx.mesh.Model, 5 , {KeepRotation = true})
		end)
		delayCompensated(5.21,function()
			Emit(Torso, Fx.space.space, 5 , {KeepRotation = true})
		end)
		
		delayCompensated(5.30,function()
			Emit(Torso, Fx["scythe cutscene"].Wave3, 5 , {KeepRotation = true})
			
			Emit(Torso, Fx["scythe cutscene"].shlam.AA, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].shlam.Color, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].shlam.Cone11, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].shlam.Idk, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].shlam.Mesh5, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].shlam.Model, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].shlam.Part, 5 , {KeepRotation = true})
			local Lines = Fx["scythe cutscene"].shlam.Lines:Clone()
			Lines.Parent = Torso
			vfx.emit(Lines)
			Debris:AddItem(Lines, 2)
			Emit(Torso, Fx["scythe cutscene"].slamfirst.groupmesh, 5 , {KeepRotation = true})
			Emit(Torso, Fx["scythe cutscene"].slamfirst.jump, 5 , {KeepRotation = true})
		end)
		delayCompensated(5.35,function()
			Emit(Torso, Fx["scythe cutscene"].Wave3, 5 , {KeepRotation = true})
		...
		delayCompensated(5.40,function()
			Emit(Torso, Fx["scythe cutscene"].Wave3, 5 , {KeepRotation = true})
		...
		delayCompensated(5.43,function()
			Emit(Camera2, Fx.screen2.black, 5 , {KeepRotation = true})
			Emit(EnemyCamera2, Fx.screen2.black, 5 , {KeepRotation = true})
		end)
		

		delayCompensated(6.23,function()
			Emit(Torso, Fx.windyboo.Model, 5 , {KeepRotation = true})
			Emit(Torso, Fx.windyboo.SPECC, 5 , {KeepRotation = true})
		end)
		
		delayCompensated(9.7,function()
			Emit(enemyTorso, Fx.stomp.stomp, 5 , {KeepRotation = true})
		end)
		
		delayCompensated(9.44,function()
			Emit(Enemy.Head, Fx.screeny.screeny, 5 , {KeepRotation = true})
		end)
		
		delayCompensated(10,function()
			Emit(Torso, Fx.spin.Wind, 5 , {KeepRotation = true})
			Emit(Torso, Fx.spin.oklol, 5 , {KeepRotation = true})
		end)
		
		delayCompensated(11.17,function()
			Emit(Enemy.Head, Fx.last.DSvfx, 5 , {KeepRotation = true})
		end)

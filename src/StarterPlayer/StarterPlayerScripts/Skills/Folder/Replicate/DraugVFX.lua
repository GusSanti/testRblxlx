...
local Effects = {
	...
	DraugUltimate = function(Params)
		...
		delayCompensated(1.2, function()
			for _, cam in ipairs({CharCamera2, EnemyCamera2}) do
				if cam then
					local ScreenLinesRun = Fx.charge.ScreenEffect.Attachment:Clone()
					ScreenLinesRun.Parent = cam

					task.delay(1.5, function()
						Utilities.Particle_Setup({Holder = cam, Type = "Emit"})
						Debris:AddItem(ScreenLinesRun, 5)
					end)
				end
			end

			Emit(Torso, Fx.attacker.TorsoEffect, 3, {KeepRotation = true})

			for _, v in pairs(Character:GetChildren()) do
				if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" then
					for _, b in pairs(Fx.attacker.aura:GetChildren()) do
						local new = b:Clone()
						new.Parent = v
						new.Enabled = true
						Debris:AddItem(new, 2)
					end
				end
			end
		end)

		delayCompensated(3.85, function()
			Emit(enemyTorso, Fx.grab.groot, 3, {KeepRotation = false})
			Emit(enemyTorso, Fx.grab.bam, 3, {KeepRotation = false})
		end)

		delayCompensated(5.5, function()
			Emit(enemyTorso, Fx.tanks.impact, 3, {KeepRotation = false})
			Emit(enemyTorso, Fx.tanks.bam, 3, {KeepRotation = false})
			Emit(enemyTorso, Fx.tanks.tnak, 3, {KeepRotation = false})
		end)

		delayCompensated(6.5, function()
			Emit(enemyTorso, Fx.slam["2"], 3, {KeepRotation = false})
			Emit(enemyTorso, Fx.slam.groot, 3, {KeepRotation = false})
			Emit(enemyTorso, Fx.slam.jump, 3, {KeepRotation = false})
		end)

		delayCompensated(8.35, function()
			Emit(enemyTorso, Fx.last.groot, 3, {KeepRotation = false})
			Emit(enemyTorso, Fx.last.jump, 3, {KeepRotation = false})
		end)
	end,
}

...
local Effects = {
	...
	TomTheTitanSharkUltimate = function(Params)
		...
		delayCompensated(0.19, function()
			local smokestart2 = Fx.SmokeStart2:Clone()
			smokestart2.Parent = Character:FindFirstChild("Left Leg")
			Debris:AddItem(smokestart2, 3)
			Utilities.Particle_Setup({Holder = smokestart2, Type = "Emit"})
			Utilities.Particle_Setup({Holder = smokestart2, Type = "Emit"})
			Utilities.Particle_Setup({Holder = smokestart2, Type = "Emit"})
			Utilities.Particle_Setup({Holder = smokestart2, Type = "Emit"})
			Utilities.Particle_Setup({Holder = smokestart2, Type = "Emit"})
		...
		delayCompensated(1.10, function()
			local FxScream = Fx.scream:Clone()
			FxScream.Parent = Character:FindFirstChild("Head")
			Debris:AddItem(FxScream, 2.15)
			Utilities.Particle_Setup({Holder = FxScream, Type = "Emit"})
			Emit(Torso, Fx.throwPart, 5, {KeepRotation = true, Y = -2.4})
			Emit(Torso, Fx.graund, 5, {KeepRotation = true, Y = -2.4})
			...
			for _, cam in ipairs({Camera2, EnemyCamera2}) do
				if cam then
					...
					Weld.Part0 = cam
					Weld.Part1 = Rachuras
					Weld.C0 = CFrame.new(1, 1, 0) * CFrame.Angles(0, math.rad(0), 0)
					Debris:AddItem(Rachuras, 2.7)
				end
			end
		end)

		delayCompensated(5.12, function()
			local boom1 = Fx.boom1:Clone()
			boom1.Parent = Torso
			Debris:AddItem(boom1, 3)
			Utilities.Particle_Setup({Holder = boom1, Type = "Emit"})
			Utilities.Particle_Setup({Holder = boom1, Type = "Emit"})
		end)
		
		delayCompensated(6.14, function()
			Emit(Torso, Fx.Windy22, 3, {KeepRotation = true, Y = 0})
		end)
		...
		delayCompensated(8.21, function()
			for _, cam in ipairs({Camera2, EnemyCamera2}) do
				if cam then
					local Rachuras = Fx.rachuras2:Clone()
					Rachuras.Parent = workspace.FX
		...
		delayCompensated(9.5, function()
			local Impact1 = Fx.impact1.bomb:Clone()
			Impact1.Parent = enemyTorso
			Utilities.Particle_Setup({Holder = Impact1, Type = "Emit"})
			Debris:AddItem(Impact1, 3)
		end)

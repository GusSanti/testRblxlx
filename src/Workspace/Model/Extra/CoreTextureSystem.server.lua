...
function TextureConfigurationSystem.AddSubDataLayer(LayerName, ParentContainer)
	...
	if ParentContainer == nil then
		warn("[TextureConfigurationSystem] - AddSubDataLayer called with nil ParentContainer for layer: " .. tostring(LayerName))
		return nil
	end
...
if RootModel == nil then
	warn("[TextureConfigurationSystem] - RootModel is nil. Initialization aborted.")
	return
end

local PoseReference = SafeWaitForChild(script, "Pose", 4)
if PoseReference and PoseReference:IsA("ObjectValue") and PoseReference.Value and PoseReference.Value:IsA("ModuleScript") then
	require(PoseReference.Value)
elseif PoseReference ~= nil then
	warn("[TextureConfigurationSystem] - Pose exists but is not an ObjectValue pointing to a ModuleScript")
end
...
if ConfigurationModel == nil then
	warn("[TextureConfigurationSystem] - Failed to create or find Configuration layer. Initialization aborted.")
	return
end

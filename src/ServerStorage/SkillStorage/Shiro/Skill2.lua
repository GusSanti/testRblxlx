...
local BARRAGE_CONFIG = {
	...
	NormalKnockback = {
		MaxForce           = Vector3.new(2e3, 0, 2e3),
...
-- ✅ NOVO: aplica AirCombatMaintainAirKnockback nos dois se ambos estiverem no ar
local function applyAirKnockback(enemyCharacter, attackerCharacter)
	task.spawn(function()
		for i = 1, 2 do
...
local function doHit(mainHitbox, Character, humRP)
	...
	mainHitbox.OnTouch = function(enemyCharacter)
		...
		if enemyplayer and player then
			if CombatBlock.CheckAndApplyCombat(enemyplayer, player, damage) then return end
		end
		...
		DamageModule.TakeDamage(enemyCharacter, damage, {StopDelay = stunTime}, Character)
		...
		-- ✅ MODIFICADO: escolhe knockback baseado se os dois estão no ar
		if CombatUtils.IsCharacterInAir(Character) and CombatUtils.IsCharacterInAir(enemyCharacter) then
			applyAirKnockback(enemyCharacter, Character)
		else
		...
		task.delay(stunTime + 0.1, function()
...
-- ✅ NOVO: pré-check hitbox antes de aplicar BodyPosition
local function PreCheckAirEnemy(char, humRP)
	local preHitbox = HitboxModule.new()
	preHitbox:AttachToCharacter(char)
	preHitbox.Size    = BARRAGE_CONFIG.HitboxSize
	preHitbox:SetOffset(BARRAGE_CONFIG.HitboxOffset)
	preHitbox.HitType = "Once"

	local foundEnemy   = nil
	local bothInAir    = false

	preHitbox.OnTouch = function(enemyCharacter)
		if foundEnemy then return end -- já achou, ignora os demais
		...
		foundEnemy = enemyCharacter
		bothInAir  = CombatUtils.IsCharacterInAir(char) and CombatUtils.IsCharacterInAir(enemyCharacter)
	end

	preHitbox:Once()
	preHitbox:Destroy()

	return foundEnemy, bothInAir
end
...
function module.UseSkill(char: Model)
	...
	local humRP    = char:FindFirstChild("HumanoidRootPart") :: Part
	...
	-- ✅ NOVO: roda o pré-check ANTES de qualquer efeito
	local foundEnemy, bothInAir = PreCheckAirEnemy(char, humRP)
	...
	-- ✅ MODIFICADO: só aplica BodyPosition se NÃO for air mode
	
	if not (foundEnemy and bothInAir) then
		CombatReplicator.BodyPosition(humRP, {
			offset   = 0.6,
			duration = 0.6,
			maxForce = Vector3.new(1, 0, 1) * 30000
		})
	else
		-- Air mode: aplica o stun de manter no ar nos dois imediatamente
		applyAirKnockback(foundEnemy, char)

		local stunTime = BARRAGE_CONFIG.NormalKnockback.Duration + 0.1
		StateManager.POST(foundEnemy, StateManagerEnums.STATES_ENUM.COMBAT_FULL_STUNNED)
		StateManager.POST_REMOVE(foundEnemy, StateManagerEnums.STATES_ENUM.COMBAT_SKILL_BEING_ATTACKED, 0.5)
		task.delay(stunTime + 0.1, function()
			StateManager.REMOVE(foundEnemy, StateManagerEnums.STATES_ENUM.COMBAT_FULL_STUNNED)
		end)
	end

	-- (resto do cleanup, loop de BEING_ATTACKED, animação, hitbox e markers — sem alteração)
	...
	hitConnection = BarrageTrack:GetMarkerReachedSignal("Hit"):Connect(function()
		...
		EffectsHelper.PlayEffect({
			...
			Delay = 0,
			Effect = Fx:FindFirstChild("Ability 2")
		}, char, false, nil, {
			C0 = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)),
			C1 = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0))
		})
	end) 

...
-- Sem KnockdownInfo → não dispara ExecuteKnockdown, não mexe em estados de knockdown
...
function module.UseSkill(char: Model)
	...
	if not animObj then
		warn("[Shiro Barrage] Animação Ability1 não encontrada em Shiro > Animations")
		return
	end
	...
	-- Substitua o CombatReplicator.BodyPosition por isso:

	local AlignAtt0 = Instance.new("Attachment")
	AlignAtt0.Name = "AlignAtt0"
	AlignAtt0.Parent = humRP

	local AlignAtt1 = Instance.new("Attachment")
	AlignAtt1.Name = "AlignAtt1"
	AlignAtt1.WorldCFrame = humRP.CFrame  -- usa WorldCFrame em vez de CFrame
	AlignAtt1.Parent = workspace.Terrain

	local AO = Instance.new("AlignOrientation")
	AO.Name = "SkillAlignOrientation"
	AO.Mode = Enum.OrientationAlignmentMode.TwoAttachment
	AO.Attachment0 = AlignAtt0
	AO.Attachment1 = AlignAtt1
	AO.Responsiveness = 200
	AO.RigidityEnabled = true  -- trava completamente, sem MaxTorque necessário
	AO.Parent = humRP

	Debris:AddItem(AO, 0.6)
	Debris:AddItem(AlignAtt0, 0.6)
	Debris:AddItem(AlignAtt1, 0.6)
	
	CombatKnockback.ApplyKnockback({Profile = KnockbackProfiles.HitPull}, Character, 0, Character)
	...
	local function cleanup()
		...
		for _, name in ipairs({"SkillLinearVelocity", "SkillAttachment", "SkillAlignOrientation", "AlignAtt0"}) do
			local inst = humRP:FindFirstChild(name)
			if inst then inst:Destroy() end
		end

		-- AlignAtt1 está no Terrain, não no humRP
		local att1 = workspace.Terrain:FindFirstChild("AlignAtt1")
	...
	if Sounds and Sounds:FindFirstChild("Ability1") then
	...
	hitConnection = RunService.Heartbeat:Connect(function()
		...
		if currentTime - lastHitboxTime >= BARRAGE_CONFIG.HitInterval then
			...
			mainHitbox.OnTouch = function(enemyCharacter)
				...
				if enemyplayer then
					if CombatBlock.CheckAndApplyCombat(enemyplayer, player, BARRAGE_CONFIG.DamagePerHit) then return end
				end
				...
				if currentTime - enemyData.lastHitTime >= BARRAGE_CONFIG.HitInterval then
					...
					DamageModule.TakeDamage(enemyCharacter, BARRAGE_CONFIG.DamagePerHit, {StopDelay = BARRAGE_CONFIG.NormalKnockback.Duration}, Character)
	...
	BarrageTrack.Stopped:Connect(function()
		if not cancelled then
			...
			finalHitbox.Size    = BARRAGE_CONFIG.HitboxSize
			finalHitbox:CreateVisualPart(false)
			...
			finalHitbox.OnTouch = function(enemyCharacter)
				...
				if enemyplayer then
					if CombatBlock.CheckAndApplyCombat(enemyplayer, player, BARRAGE_CONFIG.FinalHitDamage) then return end
				end

				DamageModule.TakeDamage(enemyCharacter, BARRAGE_CONFIG.FinalHitDamage, {StopDelay = BARRAGE_CONFIG.StunDuration}, Character)
				...
				StateManager.POST(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_FULL_STUNNED)
				task.delay(BARRAGE_CONFIG.StunDuration + 0.1, function()
					StateManager.REMOVE(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_FULL_STUNNED)
				end)
			end

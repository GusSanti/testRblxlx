...
local function applyIceEffect(enemyCharacter, duration)
	local enemyTorso = enemyCharacter:FindFirstChild("Torso")
	if not enemyTorso then return end
	...
	if iceFX then
		...
		weld.Part0  = enemyTorso
		weld.Part1  = iceFX
		weld.C0     = CFrame.new(0, 0, 0)
		weld.Parent = iceFX
	end

	StateManager.POST(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_FROZEN_STUNNED)

	local freezeSFX = Sounds:FindFirstChild("FreezeSFX") and Sounds.FreezeSFX:Clone()
	if freezeSFX then
		freezeSFX.Parent = enemyTorso
		freezeSFX:Play()
	...
	task.delay(duration, function()
		...
		StateManager.REMOVE(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_FROZEN_STUNNED)
...
local function launchSingleProjectile(char, humRP, player, state, offsetCFrame)
	...
	if not projFX then
		warn("[Bolg Ability2] FX 'Ability 2' não encontrado em Assets.FX")
		return
	end
	...
	projHitbox.OnTouch = function(enemyCharacter)
		...
		if not hitOnce[enemyId] then
			hitOnce[enemyId] = true
			applyIceEffect(enemyCharacter, CONFIG.IceStunDuration)
		end
		...
		CombatReplicator.Highlight(enemyCharacter, {Color = Color3.fromRGB(0, 180, 255), Duration = 0.5})
		DamageIndicator(enemyCharacter, damage, Color3.new(0, 0.7, 1))
...
function module.UseSkill(char: Model)
	...
	if not animObj then
		warn("[Bolg Ability2] Animação 'Ability2' não encontrada")
		return
	end

...
-- ═══════════════════════════════════════════════════════════════
local CONFIG = {
	IceMeshName        = "Spike2",           -- nome da mesh em Assets.FX
	IceFXName        = "IceFX",
	IceSpawnOffset     = CFrame.new(0, 4, -5), -- posição relativa ao HumRP
	...
	IceFinalSize       = Vector3.new(18.358, 18.358, 18.358),
...
local function spawnIceBlock(spawnCFrame, player, char)
	...
	hitbox.OnTouch = function(enemyCharacter)
		...
		task.delay(CONFIG.IceContactInterval, function()
			...
			DamageModule.TakeDamage(enemyCharacter, CONFIG.IceContactDamage, true)
...
function module.UseSkill(char: Model)
	...
	if not animObj then
		warn("[JunoIce] Animação 'Ability2' não encontrada")
		return
	end

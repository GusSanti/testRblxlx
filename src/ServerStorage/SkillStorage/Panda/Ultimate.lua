...
local function launchProjectile(char, humRP, player, state)
	local projFX = Fx:FindFirstChild("Ability 3"):Clone()
	if not projFX then
		warn("[Shiro Ability3] FX 'Ability 3' não encontrado em Assets.FX")
...
function module.UseSkill(char: Model)
	...
	local chargeFX   = Fx:FindFirstChild("Ability 3"):Clone()
	...
	local function fire()
		...
		local fireSFX = Sounds:FindFirstChild("Release") and Sounds.Release:Clone()
		if fireSFX then

...
local function launchSingleProjectile(char, humRP, player, state, offsetCFrame)
	local projFX = Fx:FindFirstChild("Super") and Fx:FindFirstChild("Super"):Clone()
	if not projFX then
		warn("[Snapper Ability1] FX 'Ability 1' não encontrado em Assets.FX")
		return
	end
...
function module.UseSkill(char: Model)
	...
	if not animObj then
		warn("[Snapper Ability] Animação 'Ability' não encontrada")
		return
	end

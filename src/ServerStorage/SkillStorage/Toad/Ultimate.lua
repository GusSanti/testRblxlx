...
local function doHit(mainHitbox, Character, humRP)
	...
	mainHitbox.OnTouch = function(enemyCharacter)
		...
		StateManager.POST(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_SKILL_BEING_ATTACKED)
		task.delay(stunTime, function()
			StateManager.REMOVE(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_FULL_STUNNED)
			StateManager.REMOVE(enemyCharacter, StateManagerEnums.STATES_ENUM.COMBAT_SKILL_BEING_ATTACKED)
		end)
	end
...
function module.UseSkill(char: Model)
	...
	if not animObj then
		warn("[Toad ultimate] Animação não encontrada")
		return
	end
	...
	local BarrageFX = Fx:FindFirstChild("Super (Melee)"):Clone()
	...
	if Sounds:FindFirstChild("Laugh") then
		state.Sfx = Sounds.Laugh:Clone()
		state.Sfx.Parent = humRP
		state.Sfx:Play()
		state.Sfx.Ended:Connect(function() state.Sfx:Destroy() end)

		state.Sfx2 = Sounds.SuperSound:Clone()
		state.Sfx2.Parent = humRP
		state.Sfx2:Play()
		state.Sfx2.Ended:Connect(function() state.Sfx2:Destroy() end)
	end

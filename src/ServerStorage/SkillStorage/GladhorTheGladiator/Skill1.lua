...
local Assets            = ReplicatedStorage.SkillStorage.Gladhor
local Fx                = Assets.FX
...
function module.UseSkill(char: Model)
	...
	local sfx = Sounds:FindFirstChild("Ability1"):Clone()
	sfx.Parent = humRP
	sfx:Play()
	...
	CombatReplicator.Emit(humRP, Fx["Ability 1"])
	...
	hitConnection = RunService.Heartbeat:Connect(function()
		mainHitbox.OnTouch = function(enemyCharacter)
			...
			CombatReplicator.Enable(enemyHumRP, Fx["Ability 2"], SKILL_CONFIG.StunTime)

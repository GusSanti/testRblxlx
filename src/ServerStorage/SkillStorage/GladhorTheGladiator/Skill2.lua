...
local PlayAnimation = require(game.ReplicatedStorage.CombatSystem.DamageModule)
...
function module.UseSkill(char: Model)
	...
	local sfx = Sounds:FindFirstChild("Ability2"):Clone()
	sfx.Parent = humRP
	sfx:Play()
	...
	task.spawn(function()
		task.wait(0.35)
		CombatReplicator.Emit(humRP, Fx["Ability 2"])
		task.wait(0.4)
		CombatReplicator.Emit(humRP, Fx["Ability 2"])
		task.wait(0.4)
		CombatReplicator.Emit(humRP, Fx["Ability 2"])

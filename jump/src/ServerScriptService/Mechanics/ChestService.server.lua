...
local function giveChestRewards(player, chestModel)
	...
	if chestModel and chestModel:IsA("Instance") then
		chestModel:Destroy()
	end
...
local function setupChestRemote()
	...
	chestRemote.OnServerInvoke = function(player, chestModel)
		...
		local rewards = giveChestRewards(player, chestModel)

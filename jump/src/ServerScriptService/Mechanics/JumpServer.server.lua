...
local function validate_world_legitimacy(player: Player): boolean
	...
	local powerOk = playerPower >= worldData.requiredPogoPower
	local rebirthsOk = playerRebirths >= worldData.requiredRebirths

	if not powerOk or not rebirthsOk then
...
local function award_coins(player: Player, combo: number, isCritical: boolean, floorMult: number)
	local rebirths = DataUtility.server.get(player, "Rebirths") or 0
	local ownedUpgrades = DataUtility.server.get(player, "OwnedRebirthUpgrades") or {}
	...
	local reward = baseCalc * rebirthMult * mult * floorMultiplier * magnetMult
...
local function handle_jump(player: Player, payload: any): ()
	if not validate_world_legitimacy(player) then return end
	...
	if character then
		...
		if layerInfo then
			if impactForce >= (layerInfo.minBreakForce or 40) then
				floorMultiplier = layerInfo.coinMultiplier
			else
				floorMultiplier = 0
			end
		end
	...
	if floorMultiplier > 0 then
		award_coins(player, 0, false, floorMultiplier)
	end
...
local function handle_rebound(player: Player, _clientCombo: number, isCritical: boolean, payload: any): ()
	...
	if not layerInfo then
		warn("[SERVER] Rebound rejected (No Layer Info):", player.Name)
		state.serverCombo = 0
	...
	local floorMultiplier = layerInfo.coinMultiplier
	...
	if floorMultiplier > 0 then
		award_coins(player, finalCombo, isCritical, floorMultiplier)
	end
